...
import { loadFixture, getPort, Vssr, Utils, rp } from '../utils'
...
// Because other response parts such as window.__VSSR may be different resulting false positive passes.
const uniqueTest = async (url) => {
  const results = []

  await Utils.parallel(range(5), async () => {
    const { html } = await vssr.renderRoute(url)
...
// Related issue: https://github.com/vssr/vssr.js/issues/1354
...
describe('ssr', () => {
  ...
  test('unique responses with vssrServerInit', async () => {
