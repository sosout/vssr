...
export default class Builder {
  constructor(vssr) {
    ...
    // Stop watching on vssr.close()
    if (this.options.dev) {
      this.vssr.hook('close', () => this.unwatch())
    }
    ...
    // this.vssr.hook('build:done', () => this.generateConfig())
    // }
  }
  ...
  normalizePlugins() {
    return _.uniqBy(
      this.options.plugins.map((p) => {
        ...
        return {
          src: this.vssr.resolveAlias(p.src),
          ssr: p.ssr !== false,
          name: 'vssr_plugin_' + pluginBaseName + '_' + hash(p.src)
        }
      }),
      p => p.name
    )
  ...
  async build() {
    ...
    // Wait for vssr ready
    await this.vssr.ready()

    // Call before hook
    await this.vssr.callHook('build:before', this, this.options.build)

    // Check if pages dir exists and warn if not
    this._vssrPages = typeof this.options.build.createRoutes !== 'function'
    if (this._vssrPages) {
      if (!fsExtra.existsSync(path.join(this.options.srcDir, this.options.dir.pages))) {
        const dir = this.options.srcDir
        if (fsExtra.existsSync(path.join(this.options.srcDir, '..', this.options.dir.pages))) {
          throw new Error(
            `No \`${this.options.dir.pages}\` directory found in ${dir}. Did you mean to run \`vssr\` in the parent (\`../\`) directory?`
          )
        } else {
    ...
    // Create .vssr/, .vssr/components and .vssr/dist folders
    ...
    // Call done hook
    await this.vssr.callHook('build:done', this)

    return this
  }

  async generateRoutesAndFiles() {
    consola.debug(`Generating vssr files`)
    ...
    let templatesFiles = [
      ...
      'utils.js',
      'empty.js',
      'components/vssr-error.vue',
      'components/vssr-loading.vue',
      'components/vssr-child.js',
      'components/vssr-link.js',
      'components/vssr.js',
    ...
    if (this._defaultPage) {
      templateVars.router.routes = createRoutes(
        ['index.vue'],
        this.options.vssrAppDir + '/pages'
      )
    } else if (this._vssrPages) { // If user defined a custom method to create routes
      // Use vssr.js createRoutes bases on pages/
      const files = {}
    ...
    templatesFiles = templatesFiles
      .map((file) => {
        ...
        return {
          src: customFileExists ? customPath : r(this.options.vssrAppDir, file),
          dst: file,
    ...
    if (this.options.loadingIndicator.name) {
      const indicatorPath1 = path.resolve(
        this.options.vssrAppDir,
        'views/loading',
        this.options.loadingIndicator.name + '.html'
      )
      const indicatorPath2 = this.vssr.resolveAlias(
    ...
    await this.vssr.callHook('build:templates', {
      templatesFiles,
      templateVars,
      resolve: r
    })

    // Interpret and move template files to .vssr/
    await Promise.all(
        ...
        try {
          const template = _.template(fileContent, {
            imports: {
              ...
              wp,
              wChunk,
              resolvePath: this.vssr.resolvePath,
              resolveAlias: this.vssr.resolveAlias,
    ...
    consola.success('Vssr files generated')
  }
  ...
  webpackCompile(compiler) {
    return new Promise(async (resolve, reject) => {
      ...
      await this.vssr.callHook('build:compile', { name, compiler })

      // Load renderer resources after build
      compiler.hooks.done.tap('load-resources', async (stats) => {
        await this.vssr.callHook('build:compiled', {
          name,
          compiler,
          stats
        })
        ...
        this.vssr.renderer.loadResources(this.mfs || fs)
  ...
  webpackDev(compiler) {
    ...
    if (this.vssr.renderer) {
      this.vssr.renderer.webpackDevMiddleware = this.webpackDevMiddleware
      this.vssr.renderer.webpackHotMiddleware = this.webpackHotMiddleware
    }
  ...
  watchServer() {
    const vssrRestartWatch = _.concat(
      this.options.serverMiddleware
        .filter(i => typeof i === 'string')
        .map(this.vssr.resolveAlias),
      this.options.watch.map(this.vssr.resolveAlias),
      path.join(this.options.rootDir, 'vssr.config.js')
    )

    this.watchers.restart = chokidar
      .watch(vssrRestartWatch, this.options.watchers.chokidar)
      .on('change', (_path) => {
        this.watchers.restart.close()
        const { name, ext } = path.parse(_path)
        this.vssr.callHook('watch:fileChanged', this, `${name}${ext}`)
      })
  }
