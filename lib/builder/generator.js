...
export default class Generator {
  ...
  async generate({ build = true, init = true } = {}) {
    ...
    // Done hook
    await this.vssr.callHook('generate:done', this, errors)

    return { errors }
  }

  async initiate({ build = true, init = true } = {}) {
    // Wait for vssr be ready
    await this.vssr.ready()

    // Call before hook
    await this.vssr.callHook('generate:before', this, this.options.generate)
  ...
  async initRoutes(...args) {
    ...
    // extendRoutes hook
    await this.vssr.callHook('generate:extendRoutes', routes)
  ...
  async afterGenerate() {
    ...
    const { html } = await this.vssr.renderRoute('/', { spa: true })
  ...
  async initDist() {
    ...
    await this.vssr.callHook('generate:distRemoved', this)
    ...
    await fsExtra.copy(this.srcBuiltPath, this.distVssrPath)

    // Add .nojekyll file to let GitHub Pages add the _vssr/ folder
    // https://help.github.com/articles/files-that-start-with-an-underscore-are-missing/
    const nojekyllPath = path.resolve(this.distPath, '.nojekyll')
    fsExtra.writeFile(nojekyllPath, '')

    await this.vssr.callHook('generate:distCopied', this)
  }
  ...
  async generateRoute({ route, payload = {}, errors = [] }) {
    ...
    try {
      const res = await this.vssr.renderer.renderRoute(route, {
        _generate: true,
        payload
      })
      html = res.html
      ...
      await this.vssr.callHook('generate:routeFailed', {
        route,
        errors: pageErrors
      })
    ...
    await this.vssr.callHook('generate:page', page)
    ...
    await this.vssr.callHook('generate:routeCreated', {
      route,
      path: page.path,
      errors: pageErrors
    })
