...
const vssrDir = fs.existsSync(path.resolve(__dirname, '..', 'package.json'))
...
export default {
  ...
  server: {
    https: false,
    port: process.env.VSSR_PORT ||
      process.env.PORT ||
      process.env.npm_package_config_vssr_port,
    host: process.env.VSSR_HOST ||
      process.env.HOST ||
  ...
  buildDir: '.vssr',
  vssrDir,
  vssrAppDir: path.resolve(vssrDir, 'lib', 'app'),
  ...
  router: {
    ...
    routes: [],
    middleware: [],
    linkActiveClass: 'vssr-link-active',
    linkExactActiveClass: 'vssr-link-exact-active',
    extendRoutes: null,
    scrollBehavior: null,
    parseQuery: false,
