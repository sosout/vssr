...
import VssrChild from './components/vssr-child.js'
import VssrLink from './components/vssr-link.js'
import VssrError from '<%= components.ErrorPage ? components.ErrorPage : "./components/vssr-error.vue" %>'
import Vssr from './components/vssr.js'
...
// Component: <vssr-child>
Vue.component(VssrChild.name, VssrChild)

// Component: <vssr-link>
Vue.component(VssrLink.name, VssrLink)

// Component: <vssr>`
Vue.component(Vssr.name, Vssr)
...
async function createApp (ssrContext) {
  ...
  const app = {
    ...
    vssr: {
      ...
      setTransitions (transitions) {
        ...
        this.$options.vssr.transitions = transitions
        return transitions
      },
      err: null,
      dateErr: null,
      error (err) {
        ...
        const vssr = this.vssr || this.$options.vssr
        vssr.dateErr = Date.now()
        vssr.err = err
        // Used in lib/server.js
        if (ssrContext) ssrContext.vssr.error = err
        return err
      }
    },
    ...App
  ...
  const inject = function (key, value) {
    ...
    const installKey = '__vssr_' + key + '_installed__'
  ...
  if (process.browser) {
    ...
    if (window.__VSSR__ && window.__VSSR__.state) {
      store.replaceState(window.__VSSR__.state)
    }
  }
  <% } %>
