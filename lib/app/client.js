...
import { createApp, VssrError } from './index'
import {
  applyAsyncData,
  sanitizeComponent,
...
Vue.config.errorHandler = (err, vm, info, ...rest) => {
  ...
  // Show Vssr Error Page
  if (vm && vm.$root && vm.$root.$vssr && vm.$root.$vssr.error && info !== 'render function') {
    vm.$root.$vssr.error(vssrError)
  }
  if (typeof defaultErrorHandler === 'function') {
    return handled
  }
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    console.error(err.message || vssrError.message)
  }
}
<% } %>
...
.then(mountApp)
.catch((err) => {
  console.error('[vssr] Error while initializing app', err)
})
...
async function loadAsyncComponents (to, from, next) {
  ...
  this._pathChanged = !!app.vssr.err || from.path !== to.path
  ...
  try {
    ...
    this.$vssr.$emit('routeChanged', to, from, err)
    next(false)
  }
}

function applySSRData(Component, ssrData) {
  if (VSSR.serverRendered && ssrData) {
...
function resolveComponents(router) {
  ...
  return flatMapComponents(router.match(path), async (Component, _, match, key, index) => {
    ...
    const _Component = applySSRData(sanitizeComponent(Component), VSSR.data ? VSSR.data[index] : null)
...
async function render (to, from, next) {
  ...
  this._dateLastError = app.vssr.dateErr
  this._hadError = !!app.vssr.err
  ...
  if (!Components.length) {
    ...
    const layout = await this.loadLayout(
      typeof VssrError.layout === 'function'
        ? VssrError.layout(app.context)
        : VssrError.layout
    )
  ...
  try {
    ...
    if (!error) {
      ...
      return this.$vssr.$emit('routeChanged', to, from, error)
    }
    _lastPaths = []
    ...
    this.error(error)
    this.$vssr.$emit('routeChanged', to, from, error)
    next(false)
  }
}
...
function showNextPage(to) {
  ...
  if (this._hadError && this._dateLastError === this.$options.vssr.dateErr) {
    this.error()
  }

  // Set layout
  let layout = this.$options.vssr.err
    ? VssrError.layout
...
function vssrReady (_app) {
  window._vssrReadyCbs.forEach((cb) => {
    if (typeof cb === 'function') {
      cb(_app)
    }
  })
  // Special JSDOM
  if (typeof window._onVssrLoaded === 'function') {
    window._onVssrLoaded(_app)
  }
  // Add router hooks
  router.afterEach((to, from) => {
    // Wait for fixPrepatch + $data updates
    Vue.nextTick(() => _app.$vssr.$emit('routeChanged', to, from))
  })
}

<% if (isDev) { %>
// Special hot reload with asyncData(context)
function getVssrChildComponents($parent, $components = []) {
  $parent.$children.forEach(($child) => {
    if ($child.$vnode.data.vssrChild && !$components.find(c =>(c.$options.__file === $child.$options.__file))) {
      $components.push($child)
    }
    if ($child.$children && $child.$children.length) {
      getVssrChildComponents($child, $components)
    }
  })
...
function hotReloadAPI (_app) {
  ...
  let $components = getVssrChildComponents(_app.$vssr, [])
...
async function mountApp(__app) {
  ...
  <% if (mode !== 'spa') { %>
    // Load layout
  const layout = VSSR.layout || 'default'
  ...
  const mount = () => {
    ...
    Vue.nextTick(() => {
      // Call window.onVssrReady callbacks
      vssrReady(_app)
      <% if (isDev) { %>
  ...
  // Enable transitions
  _app.setTransitions = _app.$options.vssr.setTransitions.bind(_app)
  if (Components.length) {
  ...
  _app.$loading = {} // To avoid error while _app.$vssr does not exist
  if (VSSR.error) _app.error(VSSR.error)
