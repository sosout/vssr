...
// window.onVssrReady(() => console.log('Ready')) hook
// Useful for jsdom testing or plugins (https://github.com/tmpvar/jsdom#dealing-with-asynchronous-script-loading)
if (process.browser) {
  window._vssrReadyCbs = []
  window.onVssrReady = (cb) => {
    window._vssrReadyCbs.push(cb)
  }
}
...
export async function setContext(app, context) {
  ...
  if (!app.context) {
    ...
    if (process.server) app.context.beforeVssrRender = fn => context.beforeRenderFns.push(fn)
    if (process.client) app.context.vssrState = window.__VSSR__
  }
  // Dynamic keys
