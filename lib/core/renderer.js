...
import defaults from '../common/vssr.config'

import MetaRenderer from './meta'
import errorMiddleware from './middleware/error'
import vssrMiddleware from './middleware/vssr'
...
export default class Renderer {
  constructor(vssr) {
    ...
    this.resources = {
      ...
      ssrTemplate: null,
      spaTemplate: null,
      errorTemplate: parseTemplate('Vssr.js Internal Server Error')
    }
  }

  async ready() {
    await this.vssr.callHook('render:before', this, this.options.render)
    // Setup vssr middleware
    ...
    // Call done hook
    await this.vssr.callHook('render:done', this)
  }
  ...
  async loadResources(_fs = fs) {
    ...
    await this.vssr.callHook('render:resourcesLoaded', this.resources)
  ...
  createRenderer() {
    ...
    this.metaRenderer = new MetaRenderer(this.vssr, this)
      ...
      Object.assign(
        {
          ...
          // for globally installed vssr command, search dependencies in global dir
  ...
  useMiddleware(m) {
    ...
    const $m = m
    if (typeof m === 'string') {
      m = this.vssr.requireModule(m)
    }
    if (typeof m.handler === 'string') {
      m.handler = this.vssr.requireModule(m.handler)
    }
  ...
  async setupMiddleware() {
    ...
    await this.vssr.callHook('render:setupMiddleware', this.app)
    ...
    // Serve .vssr/dist/ files only for production
    ...
    // Finally use vssrMiddleware
    this.useMiddleware(vssrMiddleware.bind(this))
    ...
    await this.vssr.callHook('render:errorMiddleware', this.app)

    // Apply errorMiddleware from Vssr
  ...
  async renderRoute(url, context = {}) {
    ...
    if (this.noSSR || spa) {
      ...
      const APP =
        `<div id="__vssr">${this.resources.loadingHTML}</div>` + BODY_SCRIPTS
    ...
    if (!context.vssr.serverRendered) {
      APP = '<div id="__vssr"></div>'
    }
    ...
    await this.vssr.callHook('render:routeContext', context.vssr)

    const serializedSession = `window.__VSSR__=${serialize(context.vssr, {
      isJSON: true
    })};`
  ...
  async renderAndGetWindow(url, opts = {}) {
    ...
    if (!jsdom) {
      try {
        ...
        consola.error(`
         Fail when calling vssr.renderAndGetWindow(url)
    ...
    // If Vssr could not be loaded (error from the server-side)
    const vssrExists = window.document.body.innerHTML.includes(
      this.options.render.ssr ? 'window.__VSSR__' : '<div id="__vssr">'
    )
    /* istanbul ignore if */
    if (!vssrExists) {
      const error = new Error('Could not load the vssr app')
      error.body = window.document.body.innerHTML
      throw error
    }
    // Used by vssr.js to say when the components are loaded and the app ready
    await timeout(new Promise((resolve) => {
      window._onVssrLoaded = () => resolve(window)
