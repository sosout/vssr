...
export default async function vssrMiddleware(req, res, next) {
  // Get context
  ...
  try {
    ...
    await this.vssr.callHook('render:route', req.url, result, context)
    const {
      html,
      cspScriptSrcHashSet,
      error,
    ...
    if (redirected) {
      this.vssr.callHook('render:routeDone', req.url, result, context)
      return html
    }
    if (error) {
      res.statusCode = context.vssr.error.statusCode || 500
    }
